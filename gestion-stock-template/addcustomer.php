...
<?php
  ...
  if (isset($_POST['submit'])) {
    ...
    if (empty($nom) || empty($prenom) || empty($tele) || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
      exit("<h3>Veuillez remplir tous les champs obligatoires avec des données valides.</h3>");
    }
    ...
    if (isset($_FILES["image"]) && $_FILES["image"]["name"] != "") {
      ...
      if (in_array($_FILES["image"]["type"], $allowedTypes) && $_FILES["image"]["size"] <= $maxFileSize) {
        if (move_uploaded_file($tempname, $image)) {
          ...
          $result = $client->Ajouter("client");
        } else {
          exit("<h3>Échec du téléchargement de l'image !</h3>");
        }
      } else {
        exit("<h3>Type ou taille de fichier non valide.</h3>");
      }
    } else {
      // Si aucune image n'est téléchargée, assigner une image par défaut
      $image = "./assets/img/customer/user.png";
      $client = new Client($nom, $prenom, $adr, $tele, $email, $image);
      $result = $client->Ajouter("client");
    }
    ...
    if ($result) {
      ...
      echo "<h3>Erreur lors de l'ajout du client.</h3>";
    }
  }
?>
